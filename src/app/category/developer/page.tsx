...
export default function DeveloperPage() {
  ...
  const tools = siteConfig.tools.filter(tool =>
    tool.keywords?.[language]?.includes('개발자') ||
    tool.name[language].includes('UUID') || 
    tool.name[language].includes('Base64') || 
    tool.name[language].includes('Lorem')
  )

  const content = {
    ko: {
      title: '개발자도구',
      description: '개발 작업을 위한 유용한 도구들',
      allTools: '모든 도구',
      home: '홈',
      tools: '도구',
      useBtn: '사용하기',
    ...
    en: {
      ...
      description: 'Useful tools for development work',
      allTools: 'All Tools',
      home: 'Home',
      tools: 'Tools',
      useBtn: 'Use Tool',
